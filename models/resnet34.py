...
class Resnet34_Net(torch.nn.Module):
    """
    Network for 3d Resnet
    """ 

    def __init__(self, opt):
        super(Resnet34_Net, self).__init__()
    ...
    def make_stack(self, input_conv, output_conv, length, downsample_first= False, max_pool_output= False, stride= 1):
        layers = []
        if downsample_first:
            layers.append(Resnet34_Block(input_conv,output_conv, downsample = True, stride= stride ))
        else:
            layers.append(Resnet34_Block(input_conv,output_conv, downsample = False, stride= stride ))
        
        for _ in range(length-1):
            layers.append(Resnet34_Block(output_conv,output_conv, downsample = False, stride= stride ))
...
class Resnet34_Block(torch.nn.Module):
    """
    Block for 3d Resnet
    """ 

    def __init__(self, input_filters , output_filters, downsample = False, stride=1):
        super(Resnet34_Block, self).__init__()
       
        self.conv1 = nn.Conv3d(input_filters, output_filters, kernel_size=(3,3,3), stride= stride, padding=(1,1,1), bias=False)
        self.bn1 = nn.BatchNorm3d(num_features=output_filters)
        self.conv2 = nn.Conv3d(output_filters, output_filters, kernel_size=(3,3,3),stride= stride, padding=(1,1,1), bias=False)      
